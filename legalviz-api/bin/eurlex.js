...
COMMANDS.metadata = {
  ...
  async run(args) {
    ...
    const celexUri = `http://publications.europa.eu/resource/celex/${flags.celex}`;
    const query = `
...
LIMIT 10`;
    ...
    const data = await svc.runSparqlQuery(query);
...
LIMIT 50`;
    ...
    jsonOut({ celex: flags.celex, amendments }, flags.o || flags.output);
  },
};
...
LIMIT 100`;
    ...
    jsonOut({ celex: flags.celex, acts }, flags.o || flags.output);
  },
};
